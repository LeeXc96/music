(function($){
  ...
  function Plugin( $context, options )
  {
    ...
    this.$domPlaylist     = this.$context.find(".Free--playlist");
    this.$domControls     = this.$context.find(".Free--controls");
    this.$domVolumeBar    = this.$context.find(".Free--volume");
    this.$domDetails      = this.$context.find(".Free--details");
    this.$domStatusBar    = this.$context.find(".Free--status-bar");
    this.$domProgressBar  = this.$context.find(".Free--progress-bar-wrapper");
    this.$domProgressvolume  = this.$context.find(".Free--status-volume");
    this.$domTime         = this.$context.find(".Free--time");
    this.$domElapsedTime  = this.$context.find(".Free--time-elapsed");
    this.$domTotalTime    = this.$context.find(".Free--time-total");
    this.$domThumb        = this.$context.find(".Free--thumb");
  ...
  Plugin.prototype = {
    ...
    events: function()
    {
      ...
      // - playlist events
      self.$domPlaylist.on("click", ".Free--playlist-item", function(e)
      {
        var item = $(this),
    ...
    renderPlaylist: function()
    {
      ...
      $.each(self.settings.playlist, function(i, a)
      {
        ...
        template += "<div class='Free--playlist-item' data-track='" + file + "'>";

        template += "<div class='Free--playlist-thumb'><img src='"+ thumb +"'></div>";

        template += "<div class='Free--playlist-meta-text'>";
        template += "<h4>" + trackName + "</h4>";
        template += "<p>" + trackArtist + "</p>";
        template += "</div>";
        // template += "<div class='Free--playlist-track-duration'>" + trackDuration + "</div>";
        template += "</div>";
